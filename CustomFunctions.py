...
def buy_conditions(df, n, trade_margin):
    ...
    if (lower_threshold < df.iloc[n, 4] and lower_threshold_prev > df.iloc[n + 1, 4] and df.iloc[n + 1, 12] < df.iloc[n + 1, 4]):
        return True
    # and df.iloc[n+1,12]<df.iloc[n+1, 4]*(1-trade_margin):                                              #unused since we sell everything
    elif (upper_threshold * (1 + trade_margin) < df.iloc[n, 4] and upper_threshold_prev * (1 + trade_margin) > df.iloc[n + 1, 4]):
        return True
    else:
        return False
...
def buy_conditions_MA(df, n, col_df):
    # and df.iloc[n,4]>df.iloc[n,18]:# and df.iloc[n,12]>df.iloc[n+1,12]:# and df.iloc[n+1,col_df]<df.iloc[n+2,col_df] and df.iloc[n+2,col_df]<df.iloc[n+3,col_df]:
    if df.iloc[n, 4] < df.iloc[n, 18]:
        return True
    else:
        return False
...
def sell_conditions_MA(df, n, bprice):
    # and df.iloc[n+1,col_df]>df.iloc[n+2,col_df] and df.iloc[n+2,col_df]>df.iloc[n+3,col_df]:
    if df.iloc[n, 4] > df.iloc[n, 17]:
        return True
...
def RSI_SMA(df, n, colprice, rad):  # with simple moving avg. for EMA, change after the while
    ...
    while n_count >= 0:
        if df.iloc[n_count, colprice] > df.iloc[n_count+1, colprice]:  # up
            upcounter += 1
            upsum += abs(df.iloc[n_count, colprice] -
                         df.iloc[n_count+1, colprice])
        elif df.iloc[n_count, colprice] < df.iloc[n_count+1, colprice]:
            downcounter += 1
            downsum += abs(df.iloc[n_count, colprice] -
                           df.iloc[n_count+1, colprice])
        n_count = n_count-1
...
def RSI_EMA(df, n, colprice, rad):  # with simple moving avg. for EMA, change after the while
    ...
    while n_count >= n:
        if df.iloc[n_count, colprice] > df.iloc[n_count+1, colprice]:  # up
            UPS[i] = abs(df.iloc[n_count, colprice] -
                         df.iloc[n_count+1, colprice])
            if upcounter == 0:
                EMA_up = abs(df.iloc[n_count, colprice] -
                             df.iloc[n_count+1, colprice])
            i += 1
            upcounter += 1

        elif df.iloc[n_count, colprice] < df.iloc[n_count-1, colprice]:
            DNS[j] = abs(df.iloc[n_count, colprice] -
                         df.iloc[n_count + 1, colprice])
            if downcounter == 0:
                EMA_down = abs(df.iloc[n_count, colprice] -
                               df.iloc[n_count+1, colprice])
            downcounter += 1
            j += 1
