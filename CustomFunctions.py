...
def buy_conditions(df, n, trade_margin):
    ...
    if (
        lower_threshold < df.iloc[n, 4]
        and lower_threshold_prev > df.iloc[n + 1, 4]
        and df.iloc[n + 1, 12] < df.iloc[n + 1, 4]
    ):
        return True
    # and df.iloc[n+1,12]<df.iloc[n+1, 4]*(1-trade_margin):                                              #unused since we sell everything
    elif (
        upper_threshold * (1 + trade_margin) < df.iloc[n, 4]
        and upper_threshold_prev * (1 + trade_margin) > df.iloc[n + 1, 4]
    ):
        return True
    else:
        return False
...
def RSI_SMA(
    df, n, colprice, rad
):  # with simple moving avg. for EMA, change after the while
    upcounter = 0
    downcounter = 0
    upsum = 0
    downsum = 0
...
def RSI_EMA(
    ...
    while n_count >= n:
        if df.iloc[n_count, colprice] > df.iloc[n_count + 1, colprice]:  # up
            ...
            if upcounter == 0:
                EMA_up = abs(
                    df.iloc[n_count, colprice] - df.iloc[n_count + 1, colprice]
                )
            i += 1
            upcounter += 1
